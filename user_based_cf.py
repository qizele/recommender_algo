...
#user-based CF recommender (KNN)
def user_based_cf(data_file, N, null_value):
    ...
    #Calculate consine similarity and predict for each users (for small RAM)
    for i in data_file_imputed.index:
        # calculate the cosine similarity for two vector
        cos_list=[]
        for j in data_file_imputed.index:
        ...
        cosine_similarity=pd.DataFrame(cos_list, index=data_file_imputed.index, columns=['similarity'])
        ...
        nom=KNN_matrix.multiply(np.array(list(KNN_similarity['similarity'])), axis=0).sum(axis=0)
        denom=KNN_similarity['similarity'].abs().sum()
        pred=user_mean[i]+nom.divide(denom)
        data_file_predict.loc[i,:]=pred

    return data_file_predict


user_based_predict=user_based_cf(sample_data, 80, 99)
...
nmae=cf_nmae(sample_data, 99, user_based_predict, 10, -10)
print(nmae)
